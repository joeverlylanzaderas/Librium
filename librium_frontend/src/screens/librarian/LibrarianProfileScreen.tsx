// src/screens/librarian/LibrarianProfileScreen.tsx
...
export default function LibrarianProfileScreen() {
  ...
  const sexLabel =
    me.profile?.sex === 'M' ? 'Male' :
    me.profile?.sex === 'F' ? 'Female' :
    me.profile?.sex === 'O' ? 'Other' : '—';
  ...
  return (
    <ScrollView style={s.container} contentContainerStyle={s.scrollContent}>
      ...
      <View style={s.headerCard}>
          ...
          {me.profile?.profile_picture ? (
            ...
            <View style={s.avatarContainer}>
              <Text style={s.avatarText}>
                {me.full_name?.charAt(0)?.toUpperCase() ?? 'L'}
              </Text>
            </View>
          )}
        ...
        <Text style={s.userName}>{me.full_name}</Text>
        <Text style={s.userEmail}>{me.email}</Text>
        <View style={s.roleBadge}>
          <Text style={s.roleText}>LIBRARIAN</Text>
        </View>
      </View>
      ...
      <View style={s.section}>
        ...
        <View style={s.infoRow}><Text style={s.infoLabel}>Age</Text><Text style={s.infoValue}>{me.profile?.age ?? '—'}</Text></View>
        <View style={s.infoRow}><Text style={s.infoLabel}>Sex</Text><Text style={s.infoValue}>{sexLabel}</Text></View>
      ...
      <View style={s.section}>
        ...
        <Text style={s.bodyText}>{me.profile?.bio || 'No biography configured.'}</Text>
      </View>
      ...
      <View style={s.actionLayout}>
        <TouchableOpacity style={s.btnPrimary} onPress={openEditModal}>
          ...
          <Text style={s.btnPrimaryTxt}>Modify Profile Details</Text>
        </TouchableOpacity>

        <TouchableOpacity style={s.btnSecondary} onPress={() => setPwModal(true)}>
          <Ionicons name="lock-closed-outline" size={18} color={P.parchmentDark} />
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={s.modalOverlay}>
          <View style={s.modalSheet}>
            <View style={s.modalHeader}>
              <Text style={s.modalTitle}>Modify Profile</Text>
              <TouchableOpacity onPress={() => setEditModal(false)}>
                <Ionicons name="close" size={24} color={P.parchment} />
              </TouchableOpacity>
            </View>

            <ScrollView showsVerticalScrollIndicator contentContainerStyle={s.modalSheetContent}>
              ...
              <Text style={s.inputLabel}>Biography</Text>
              ...
              <TouchableOpacity style={s.modalSubmitBtn} onPress={handleSaveProfile} disabled={saving}>
                {saving
                  ? <ActivityIndicator color={P.espresso} />
                  : <Text style={s.modalSubmitBtnTxt}>Save Changes</Text>
                }
              </TouchableOpacity>
            </ScrollView>
          </View>
      ...
      <Modal visible={pwModal} animationType="fade" transparent onRequestClose={() => setPwModal(false)}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={s.modalOverlay}>
          <View style={[s.modalSheet, { maxWidth: 500 }]}>
            ...
            <ScrollView showsVerticalScrollIndicator contentContainerStyle={s.modalSheetContent}>
              ...
              <TouchableOpacity style={s.modalSubmitBtn} onPress={handleChangePassword} disabled={changingPw}>
                {changingPw
                  ? <ActivityIndicator color={P.espresso} />
                  : <Text style={s.modalSubmitBtnTxt}>Update Password</Text>
                }
              </TouchableOpacity>
            </ScrollView>
          </View>
...
const s = StyleSheet.create({
  ...
  centerContainer: { flex: 1, minHeight: 400, alignItems: 'center', justifyContent: 'center' },
  ...
  avatarImage:     { width: 84, height: 84, borderRadius: 42, borderWidth: 2, borderColor: P.brass },
  avatarContainer: { width: 84, height: 84, borderRadius: 42, backgroundColor: P.brass, alignItems: 'center', justifyContent: 'center' },
  avatarEditBadge: {
    position: 'absolute', bottom: 0, right: 0,
    backgroundColor: P.brass, borderRadius: 12,
    width: 24, height: 24,
    alignItems: 'center', justifyContent: 'center',
    borderWidth: 2, borderColor: P.espresso,
  },
  avatarText:  { color: P.espresso, fontSize: 34, fontWeight: '800', fontFamily: SERIF_FONT },
  userName:    { color: P.parchment, fontSize: 22, fontWeight: '700', fontFamily: SERIF_FONT, marginBottom: 4 },
  userEmail:   { color: P.parchmentDark, fontSize: 14, marginBottom: 14, opacity: 0.8 },
  roleBadge:   { backgroundColor: 'rgba(255,200,92,0.15)', borderRadius: 20, paddingHorizontal: 16, paddingVertical: 6, borderWidth: 1, borderColor: P.brass },
  roleText:    { color: P.brass, fontSize: 11, fontWeight: '800', letterSpacing: 1 },

  section:      { backgroundColor: P.espresso, borderRadius: 12, borderWidth: 1, borderColor: P.mahogany, padding: 20 },
  sectionTitle: { color: P.brass, fontSize: 12, fontWeight: '700', letterSpacing: 1.2, textTransform: 'uppercase', borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.05)', paddingBottom: 8, marginBottom: 12 },
  infoRow:      { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.03)' },
  ...
  btnPrimary:   { backgroundColor: P.brass, borderRadius: 8, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  btnPrimaryTxt:{ color: P.espresso, fontWeight: '700', fontSize: 14 },
  btnSecondary: { backgroundColor: 'rgba(255,255,255,0.05)', borderRadius: 8, borderWidth: 1, borderColor: P.mahogany, paddingVertical: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  btnSecondaryTxt: { color: P.parchmentDark, fontWeight: '600', fontSize: 14 },
  btnDanger:    { backgroundColor: 'rgba(255,107,107,0.08)', borderRadius: 8, borderWidth: 1, borderColor: 'rgba(255,107,107,0.2)', paddingVertical: 14, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8 },
  ...
  modalSheet:        { backgroundColor: P.espresso, borderRadius: 12, borderWidth: 1, borderColor: P.mahogany, width: '100%', maxWidth: 600, alignSelf: 'center', maxHeight: '85%', overflow: 'hidden' },
  modalHeader:       { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 24, paddingTop: 20, paddingBottom: 16, borderBottomWidth: 1, borderBottomColor: 'rgba(255,255,255,0.08)', backgroundColor: P.espresso },
  modalSheetContent: { paddingHorizontal: 24, paddingTop: 16, paddingBottom: 32 },
  ...
  input:             { backgroundColor: 'rgba(0,0,0,0.25)', borderWidth: 1, borderColor: '#543D20', borderRadius: 6, paddingHorizontal: 14, paddingVertical: 12, fontSize: 14, color: '#FFFFFF', marginBottom: 16 },
  ...
  modalSubmitBtn:    { backgroundColor: P.brass, borderRadius: 6, paddingVertical: 14, alignItems: 'center', marginTop: 8 },
